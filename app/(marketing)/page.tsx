import { buttonVariants } from "@/components/ui/button";
import { siteConfig } from "@/config/site";
import { cn } from "@/lib/utils";
import Link from "next/link";

export default function IndexPage() {
    return (
        <>
            <section className="px-4 pt-6 md:pt-10 lg:pt-20 pb-8 md:pb-12">
                <div className="container mx-auto text-center flex flex-col items-center gap-4 max-w-[64rem]">
                    <h1 className="font-extrabold text-3xl sm:text-5xl md:text-6xl lg:text-7xl">Shimon's Tech Blog</h1>
                    <p className="text-muted-foreground sm:text-xl leading-normal max-wide-[42rem]">ソフトウェアエンジニア 史門のテックブログです。</p>
                    <div className="space-x-4">
                        {/* <Link href={"/login"} className={cn(buttonVariants({ size: "lg", variant: "outline"}))}>はじめる</Link> */}
                        <Link
                            href={siteConfig.links.x}
                            className={cn(buttonVariants({ size: "lg"}))}
                            target="_blank"
                            rel="noreferrer"
                        >
                            Xをフォロー
                        </Link>
                        <Link href={siteConfig.links.github} className={cn(buttonVariants({ size: "lg" }))} target="_blank" rel="noreferrer">GitHub</Link>
                    </div>
                </div>
            </section>

            <section id="about-me" className="container mx-auto px-4 py-8 md:py-12 lg:py-24 bg-slate-50 space-y-6">
                <div className="space-y-6 max-w-[58rem] mx-auto">
                    <h2 className="font-extrabold text-3xl md:text-6xl text-center">
                        About Me
                    </h2>
                    <p className="text-muted-foreground sm:text-lg sm:leading-7 text-left">
                        ソフトウェアエンジニアの Shimon です。現在、事業会社の新規事業部にてSaaSの開発を行っています。
                    </p>
                    <p className="text-muted-foreground sm:text-lg sm:leading-7 text-left">
                        PHP, Laravel, Goを用いたバックエンドの開発が強みです。
                        TypeScript, Reactを用いたフロントエンドの開発経験もあるため、フルスタックな開発が可能です。
                    </p>
                    <p className="text-muted-foreground sm:text-lg sm:leading-7 text-left">
                        このブログでは、日々の学びや技術的なメモを記事として残しています。また、技術的な記事の他にも、技術書を読んだ際の学びやキャリアなどについてもアウトプットしています。
                    </p>
                </div>
                <div className="mx-auto text-center gap-4 max-w-[64rem] pt-6">
                    <h3 className="font-bold text-3xl md:text-5xl">Skills</h3>
                    <p className="text-muted-foreground sm:text-lg sm:leading-7 text-center pt-4">
                        下記の技術を用いた開発経験があります。
                    </p>
                </div>
                <div className="mx-auto grid grid-cols-3 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-7 gap-4 max-w-[64rem]">
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="45"
                                height="45"
                                viewBox="0 0 128 128"
                                className="flex-shrink-0"
                            >
                                <path fill="url(#deviconPhp0)" d="M0 64c0 18.593 28.654 33.667 64 33.667c35.346 0 64-15.074 64-33.667c0-18.593-28.655-33.667-64-33.667C28.654 30.333 0 45.407 0 64Z" />
                                <path fill="#777bb3" d="M64 95.167c33.965 0 61.5-13.955 61.5-31.167c0-17.214-27.535-31.167-61.5-31.167S2.5 46.786 2.5 64c0 17.212 27.535 31.167 61.5 31.167Z" />
                                <path d="M34.772 67.864c2.793 0 4.877-.515 6.196-1.53c1.306-1.006 2.207-2.747 2.68-5.175c.44-2.27.272-3.854-.5-4.71c-.788-.874-2.493-1.317-5.067-1.317h-4.464l-2.473 12.732zM20.173 83.547a.694.694 0 0 1-.68-.828l6.557-33.738a.695.695 0 0 1 .68-.561h14.134c4.442 0 7.748 1.206 9.827 3.585c2.088 2.39 2.734 5.734 1.917 9.935c-.333 1.711-.905 3.3-1.7 4.724a15.818 15.818 0 0 1-3.128 3.92c-1.531 1.432-3.264 2.472-5.147 3.083c-1.852.604-4.232.91-7.07.91h-5.724l-1.634 8.408a.695.695 0 0 1-.682.562z" />
                                <path fill="#fff" d="M34.19 55.826h3.891c3.107 0 4.186.682 4.553 1.089c.607.674.723 2.097.331 4.112c-.439 2.257-1.253 3.858-2.42 4.756c-1.194.92-3.138 1.386-5.773 1.386h-2.786l2.205-11.342zm6.674-8.1H26.731a1.39 1.39 0 0 0-1.364 1.123L18.81 82.588a1.39 1.39 0 0 0 1.363 1.653h7.35a1.39 1.39 0 0 0 1.363-1.124l1.525-7.846h5.151c2.912 0 5.364-.318 7.287-.944c1.977-.642 3.796-1.731 5.406-3.237a16.522 16.522 0 0 0 3.259-4.087c.831-1.487 1.429-3.147 1.775-4.931c.86-4.423.161-7.964-2.076-10.524c-2.216-2.537-5.698-3.823-10.349-3.823zM30.301 68.557h4.471c2.963 0 5.17-.557 6.62-1.675c1.451-1.116 2.428-2.98 2.938-5.591c.485-2.508.264-4.277-.665-5.308c-.931-1.03-2.791-1.546-5.584-1.546h-5.036l-2.743 14.12m10.563-19.445c4.252 0 7.353 1.117 9.303 3.348c1.95 2.232 2.536 5.347 1.76 9.346c-.322 1.648-.863 3.154-1.625 4.518c-.764 1.366-1.76 2.614-2.991 3.747c-1.468 1.373-3.097 2.352-4.892 2.935c-1.794.584-4.08.875-6.857.875h-6.296l-1.743 8.97h-7.35l6.558-33.739h14.133" />
                                <path d="M69.459 74.577a.694.694 0 0 1-.682-.827l2.9-14.928c.277-1.42.209-2.438-.19-2.87c-.245-.263-.979-.704-3.15-.704h-5.256l-3.646 18.768a.695.695 0 0 1-.683.56h-7.29a.695.695 0 0 1-.683-.826l6.558-33.739a.695.695 0 0 1 .682-.561h7.29a.695.695 0 0 1 .683.826L64.41 48.42h5.653c4.307 0 7.227.758 8.928 2.321c1.733 1.593 2.275 4.14 1.608 7.573l-3.051 15.702a.695.695 0 0 1-.682.56h-7.407z" />
                                <path fill="#fff" d="M65.31 38.755h-7.291a1.39 1.39 0 0 0-1.364 1.124l-6.557 33.738a1.39 1.39 0 0 0 1.363 1.654h7.291a1.39 1.39 0 0 0 1.364-1.124l3.537-18.205h4.682c2.168 0 2.624.463 2.641.484c.132.14.305.795.019 2.264l-2.9 14.927a1.39 1.39 0 0 0 1.364 1.654h7.408a1.39 1.39 0 0 0 1.363-1.124l3.051-15.7c.715-3.686.103-6.45-1.82-8.217c-1.836-1.686-4.91-2.505-9.398-2.505h-4.81l1.421-7.315a1.39 1.39 0 0 0-1.364-1.655zm0 1.39l-1.743 8.968h6.496c4.087 0 6.907.714 8.457 2.14c1.553 1.426 2.017 3.735 1.398 6.93l-3.052 15.699h-7.407l2.901-14.928c.33-1.698.208-2.856-.365-3.474c-.573-.617-1.793-.926-3.658-.926h-5.829l-3.756 19.327H51.46l6.558-33.739h7.292z" />
                                <path d="M92.136 67.864c2.793 0 4.878-.515 6.198-1.53c1.304-1.006 2.206-2.747 2.679-5.175c.44-2.27.273-3.854-.5-4.71c-.788-.874-2.493-1.317-5.067-1.317h-4.463l-2.475 12.732zM77.54 83.547a.694.694 0 0 1-.682-.828l6.557-33.738a.695.695 0 0 1 .682-.561H98.23c4.442 0 7.748 1.206 9.826 3.585c2.089 2.39 2.734 5.734 1.917 9.935a15.878 15.878 0 0 1-1.699 4.724a15.838 15.838 0 0 1-3.128 3.92c-1.53 1.432-3.265 2.472-5.147 3.083c-1.852.604-4.232.91-7.071.91h-5.723l-1.633 8.408a.695.695 0 0 1-.683.562z" />
                                <path fill="#fff" d="M91.555 55.826h3.891c3.107 0 4.186.682 4.552 1.089c.61.674.724 2.097.333 4.112c-.44 2.257-1.254 3.858-2.421 4.756c-1.195.92-3.139 1.386-5.773 1.386h-2.786l2.204-11.342zm6.674-8.1H84.096a1.39 1.39 0 0 0-1.363 1.123l-6.558 33.739a1.39 1.39 0 0 0 1.364 1.653h7.35a1.39 1.39 0 0 0 1.363-1.124l1.525-7.846h5.15c2.911 0 5.364-.318 7.286-.944c1.978-.642 3.797-1.731 5.408-3.238a16.52 16.52 0 0 0 3.258-4.086c.832-1.487 1.428-3.147 1.775-4.931c.86-4.423.162-7.964-2.076-10.524c-2.216-2.537-5.697-3.823-10.35-3.823zM87.666 68.557h4.47c2.964 0 5.17-.557 6.622-1.675c1.45-1.116 2.428-2.98 2.936-5.591c.487-2.508.266-4.277-.665-5.308c-.93-1.03-2.791-1.546-5.583-1.546h-5.035Zm10.563-19.445c4.251 0 7.354 1.117 9.303 3.348c1.95 2.232 2.537 5.347 1.759 9.346c-.32 1.648-.862 3.154-1.624 4.518c-.763 1.366-1.76 2.614-2.992 3.747c-1.467 1.373-3.097 2.352-4.892 2.935c-1.793.584-4.078.875-6.856.875h-6.295l-1.745 8.97h-7.35l6.558-33.739h14.133" />
                                <defs>
                                    <radialGradient id="deviconPhp0" cx="0" cy="0" r="1" gradientTransform="translate(38.426 42.169) scale(84.04136)" gradientUnits="userSpaceOnUse">
                                        <stop stop-color="#AEB2D5" />
                                        <stop offset=".3" stop-color="#AEB2D5" />
                                        <stop offset=".75" stop-color="#484C89" />
                                        <stop offset="1" stop-color="#484C89" />
                                    </radialGradient>
                                </defs>
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-xl">PHP</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="45"
                                height="45"
                                viewBox="0 0 256 264"
                                className="flex-shrink-0"
                            >
                                <path fill="#FF2D20" d="M255.856 59.62c.095.351.144.713.144 1.077v56.568c0 1.478-.79 2.843-2.073 3.578L206.45 148.18v54.18a4.135 4.135 0 0 1-2.062 3.579l-99.108 57.053c-.227.128-.474.21-.722.299c-.093.03-.18.087-.278.113a4.15 4.15 0 0 1-2.114 0c-.114-.03-.217-.093-.325-.134c-.227-.083-.464-.155-.68-.278L2.073 205.938A4.128 4.128 0 0 1 0 202.36V32.656c0-.372.052-.733.144-1.083c.031-.119.103-.227.145-.346c.077-.216.15-.438.263-.639c.077-.134.19-.242.283-.366c.119-.165.227-.335.366-.48c.119-.118.274-.206.408-.309c.15-.124.283-.258.453-.356h.005L51.613.551a4.135 4.135 0 0 1 4.125 0l49.546 28.526h.01c.165.104.305.232.454.351c.134.103.284.196.402.31c.145.149.248.32.371.484c.088.124.207.232.279.366c.118.206.185.423.268.64c.041.118.113.226.144.35c.095.351.144.714.145 1.078V138.65l41.286-23.773V60.692c0-.36.052-.727.145-1.072c.036-.124.103-.232.144-.35c.083-.217.155-.44.268-.64c.077-.134.19-.242.279-.366c.123-.165.226-.335.37-.48c.12-.118.269-.206.403-.309c.155-.124.289-.258.454-.356h.005l49.551-28.526a4.13 4.13 0 0 1 4.125 0l49.546 28.526c.175.103.309.232.464.35c.128.104.278.197.397.31c.144.15.247.32.37.485c.094.124.207.232.28.366c.118.2.185.423.267.64c.047.118.114.226.145.35Zm-8.115 55.258v-47.04l-17.339 9.981l-23.953 13.792v47.04l41.297-23.773h-.005Zm-49.546 85.095V152.9l-23.562 13.457l-67.281 38.4v47.514l90.843-52.3ZM8.259 39.796v160.177l90.833 52.294v-47.505L51.64 177.906l-.015-.01l-.02-.01c-.16-.093-.295-.227-.444-.34c-.13-.104-.279-.186-.392-.3l-.01-.015c-.134-.129-.227-.289-.34-.433c-.104-.14-.227-.258-.31-.402l-.005-.016c-.093-.154-.15-.34-.217-.515c-.067-.155-.154-.3-.196-.464v-.005c-.051-.196-.061-.403-.082-.604c-.02-.154-.062-.309-.062-.464V63.57L25.598 49.772l-17.339-9.97v-.006ZM53.681 8.893L12.399 32.656l41.272 23.762L94.947 32.65L53.671 8.893h.01Zm21.468 148.298l23.948-13.786V39.796L81.76 49.778L57.805 63.569v103.608l17.344-9.986ZM202.324 36.935l-41.276 23.762l41.276 23.763l41.271-23.768l-41.27-23.757Zm-4.13 54.676l-23.953-13.792l-17.338-9.981v47.04l23.948 13.787l17.344 9.986v-47.04Zm-94.977 106.006l60.543-34.564l30.264-17.272l-41.246-23.747l-47.489 27.34l-43.282 24.918l41.21 23.325Z" />
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-xl">Laravel</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="45"
                                height="45"
                                viewBox="0 0 128 128"
                                className="flex-shrink-0"
                            >
                                <defs>
                                    <path id="deviconGo0" d="M18.8 1h90.5v126H18.8z" />
                                </defs>
                                <clipPath id="deviconGo1">
                                    <use href="#deviconGo0" />
                                </clipPath>
                                <path fill="#F6D2A2" fill-rule="evenodd" d="M21.1 68.7c.2 3.5 3.7 1.9 5.3.8c1.5-1.1 2-.2 2.1-2.3c.1-1.4.2-2.7.2-4.1c-2.3-.2-4.8.3-6.7 1.7c-.9.7-2.8 3-.9 3.9" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path d="M23 71.2c-.7 0-2-.3-2.2-2.3c-.6-.4-.8-.9-.8-1.2c-.1-1.2 1.2-2.6 1.9-3.1c1.6-1.2 3.7-1.8 5.9-1.8h1.3v.3c.1 1.1 0 2.2-.1 3.2c0 .3 0 .6-.1.9c-.1 1.5-.4 1.7-1.1 2c-.3.1-.6.2-1.1.6c-.5.3-2.2 1.4-3.7 1.4zm4.8-7.8c-2.1 0-4 .6-5.5 1.7c-.7.5-1.7 1.7-1.6 2.5c0 .3.2.6.6.8l.2.1v.2c.1 1.6.9 1.8 1.5 1.8c1 0 2.4-.7 3.3-1.3c.6-.4 1-.5 1.3-.6c.5-.2.6-.2.7-1.4c0-.3 0-.6.1-.9c.1-.9.1-1.9.1-2.8c-.3-.1-.5-.1-.7-.1z" clip-path="url(#deviconGo1)" />
                                <path fill="#C6B198" fill-rule="evenodd" d="M21.1 68.7c.5-.2 1.1-.3 1.4-.8" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path d="M21.1 69c-.1 0-.3-.1-.3-.2c-.1-.2 0-.4.2-.4c.1 0 .2-.1.2-.1c.4-.2.8-.3 1-.6c.1-.1.3-.2.5-.1c.1.1.2.3.1.5c-.4.5-.9.7-1.3.8l-.2.1h-.2z" clip-path="url(#deviconGo1)" />
                                <path fill="#6AD7E5" fill-rule="evenodd" d="M29.3 26.4c-13.6-3.8-3.5-21.1 7.4-14l-7.4 14z" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path d="m29.5 26.8l-.3-.1c-7-2-6.9-7-6.7-8.5c.5-3.8 4.1-7.8 8.9-7.8c1.9 0 3.7.6 5.5 1.8l.3.2l-7.7 14.4zm1.9-15.7c-4.5 0-7.8 3.7-8.3 7.2c-.5 3.6 1.7 6.4 6 7.7l7.1-13.5c-1.5-.9-3.1-1.4-4.8-1.4z" clip-path="url(#deviconGo1)" />
                                <path fill="#6AD7E5" fill-rule="evenodd" d="M89.6 11.1c10.7-7.5 20.5 9.5 8 13.8l-8-13.8z" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path d="M97.5 25.3L89.2 11l.3-.2c1.9-1.3 3.8-2 5.7-2c4.6 0 7.9 3.8 8.6 7.5c.3 1.5.6 6.6-6 8.8l-.3.2zm-7.4-14l7.7 13.3c3.9-1.4 5.9-4.4 5.3-8c-.6-3.4-3.7-6.9-7.9-6.9c-1.7-.1-3.4.4-5.1 1.6z" clip-path="url(#deviconGo1)" />
                                <path fill="#F6D2A2" fill-rule="evenodd" d="M92 112.3c2.7 1.7 7.7 6.8 3.6 9.3c-3.9 3.6-6.1-4-9.6-5c1.5-2 3.4-3.9 6-4.3" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path d="M93.5 122.9c-1.6 0-3-1.6-4.2-3.1c-1.1-1.2-2.2-2.5-3.4-2.9l-.5-.1l.3-.4c1.2-1.7 3.2-3.9 6.2-4.4h.1l.1.1c1.7 1.1 5.4 4.2 5.3 7.1c0 1.1-.6 2-1.7 2.7c-.7.7-1.4 1-2.2 1zm-7-6.5c1.2.5 2.2 1.8 3.2 2.9c1.2 1.5 2.4 2.8 3.7 2.8c.6 0 1.2-.3 1.8-.9h.1c.9-.6 1.4-1.3 1.4-2.2c0-2.3-2.9-5.2-4.9-6.5c-1.8.5-3.6 1.7-5.3 3.9zm9.1 5.5c-.1 0-.2-.1-.3-.2c-.2-.4-.4-.9-.5-1.3c-.3-.8-.6-1.6-1.2-2.2c-.1-.1-.1-.3 0-.5c.1-.1.3-.1.5 0c.7.7 1.1 1.6 1.4 2.5l.5 1.2c.1.2 0 .4-.1.5h-.3z" clip-path="url(#deviconGo1)" />
                                <path fill="#F6D2A2" fill-rule="evenodd" d="M43.2 118.1c-3.2.5-5 3.4-7.7 4.9c-2.5 1.5-3.5-.5-3.7-.9c-.4-.2-.4.2-1-.4c-2.3-3.7 2.4-6.4 4.9-8.2c3.5-.8 5.7 2.2 7.5 4.6" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path d="M33.8 123.8c-1.3 0-2-1.1-2.2-1.5h-.1c-.3 0-.5-.1-.9-.5v-.1c-2.2-3.5 1.6-6.2 4.1-8l.9-.6h.2c.4-.1.7-.1 1.1-.1c3 0 4.9 2.6 6.5 4.7l.5.7l-.6.1c-1.9.3-3.3 1.5-4.7 2.7c-.9.8-1.8 1.5-2.8 2.1c-.8.3-1.4.5-2 .5zm-2.2-2.1c.1 0 .2 0 .4.1h.1l.1.1c.2.3.7 1.2 1.7 1.2c.5 0 1-.2 1.5-.5c1-.5 1.9-1.3 2.7-2c1.3-1.1 2.7-2.3 4.5-2.8c-1.5-2-3.3-4.2-5.8-4.2c-.3 0-.6 0-.9.1l-.8.6c-2.6 1.8-5.8 4.1-3.9 7.1c.1.2.2.3.4.3zm.2.7c-.2 0-.4-.2-.3-.4c.1-1 .6-1.7 1.1-2.5c.3-.4.5-.8.7-1.2c.1-.2.3-.2.4-.2c.2.1.2.3.2.4c-.2.5-.5.9-.8 1.3c-.5.7-.9 1.3-1 2.1c0 .4-.1.5-.3.5z" clip-path="url(#deviconGo1)" />
                                <path fill-rule="evenodd" d="M29.9 21.7c-1.8-.9-3.1-2.2-2-4.3c1-1.9 2.9-1.7 4.7-.8l-2.7 5.1zm64.9-1.8c1.8-.9 3.1-2.2 2-4.3c-1-1.9-2.9-1.7-4.7-.8l2.7 5.1z" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path fill="#F6D2A2" fill-rule="evenodd" d="M107.1 68.2c-.2 3.5-3.7 1.9-5.3.8c-1.5-1.1-2-.2-2.1-2.3c-.1-1.4-.2-2.7-.2-4.1c2.3-.2 4.8.3 6.7 1.7c1 .8 2.8 3 .9 3.9" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path d="M105.3 70.7c-1.5 0-3.2-1.1-3.7-1.4c-.5-.3-.8-.5-1.1-.6c-.8-.3-1-.5-1.1-2c0-.3 0-.6-.1-.9c-.1-1-.2-2.1-.1-3.2v-.3h1.3c2.2 0 4.3.6 5.9 1.8c.7.5 2 1.9 1.9 3.1c0 .4-.2.9-.8 1.2c-.2 2-1.5 2.3-2.2 2.3zM99.8 63c0 .9 0 1.9.1 2.8c0 .3 0 .6.1.9c.1 1.2.2 1.2.7 1.4c.3.1.7.3 1.3.6c.9.6 2.3 1.3 3.3 1.3c.6 0 1.4-.2 1.5-1.8V68l.2-.1c.4-.2.6-.4.6-.8c.1-.8-.9-2-1.6-2.5c-1.5-1.1-3.5-1.7-5.5-1.7c-.2.1-.4.1-.7.1z" clip-path="url(#deviconGo1)" />
                                <path fill="#C6B198" fill-rule="evenodd" d="M107.1 68.2c-.5-.2-1.1-.3-1.4-.8" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path d="M107.1 68.6h-.1l-.2-.1c-.5-.2-1-.3-1.3-.8c-.1-.1-.1-.4.1-.5c.1-.1.4-.1.5.1c.2.3.6.4 1 .6c.1 0 .2.1.2.1c.2.1.3.3.2.4c-.1.1-.3.2-.4.2z" clip-path="url(#deviconGo1)" />
                                <path fill="#6AD7E5" fill-rule="evenodd" d="M62.8 4c13.6 0 26.3 1.9 33 15c6 14.6 3.8 30.4 4.8 45.9c.8 13.3 2.5 28.6-3.6 40.9c-6.5 12.9-22.7 16.2-36 15.7c-10.5-.4-23.1-3.8-29.1-13.4c-6.9-11.2-3.7-27.9-3.2-40.4c.6-14.8-4-29.7.9-44.1C34.5 8.5 48.1 5.1 62.8 4" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path d="M63.3 121.9h-2.5c-4.1-.1-10.3-.8-16.4-3.3c-5.9-2.4-10.2-5.8-13-10.3c-5.6-9.1-4.6-21.6-3.7-32.7c.2-2.8.4-5.4.5-7.9c.2-5.2-.2-10.6-.7-15.7c-.8-9.4-1.6-19.1 1.5-28.5c2.4-7 6.7-12 13.2-15.2c5.1-2.5 11.4-3.9 20.4-4.6C76 3.6 89.3 5.5 96 18.8c4.4 10.7 4.4 22.2 4.5 33.3c0 4.2 0 8.5.3 12.7c.1 1.3.2 2.6.2 3.9c.8 12.2 1.7 26-3.9 37.2c-2.8 5.7-7.7 9.9-14.4 12.6c-5.4 2.2-12.2 3.4-19.4 3.4zM62.8 4.3c-14.1 1.1-27.9 4.2-33 19.4c-3.1 9.3-2.3 18.9-1.5 28.2c.4 5.2.9 10.5.7 15.8c-.1 2.5-.3 5.1-.5 7.9c-.9 11-1.9 23.4 3.6 32.3c2.3 3.7 9.7 12.5 28.8 13.2h2.5c22.1 0 30.3-9.8 33.3-15.6c5.5-11 4.6-24.8 3.9-36.9c-.1-1.3-.2-2.6-.2-3.9c-.2-4.2-.3-8.5-.3-12.7c-.1-11-.1-22.5-4.4-33.1C92.7 13 88.2 9 82 6.7c-6.4-2.1-13.6-2.4-19.2-2.4z" clip-path="url(#deviconGo1)" />
                                <path fill="#fff" fill-rule="evenodd" d="M65.2 22.2c2.4 14.2 25.6 10.4 22.3-3.9c-3-12.8-23.1-9.2-22.3 3.9" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path d="M76.2 31.5c-4.5 0-10.2-2.4-11.4-9.2c-.2-3.2.8-6.1 2.9-8.3c2.3-2.5 5.8-3.9 9.4-3.9c4.2 0 9.2 2.2 10.6 8.3c.8 3.4.2 6.4-1.7 8.8c-2.1 2.6-5.8 4.3-9.8 4.3zm-10.7-9.3c.5 2.8 1.8 5 3.9 6.6c1.8 1.4 4.3 2.1 6.8 2.1c3.7 0 7.3-1.6 9.3-4.1c1.8-2.2 2.3-5.1 1.6-8.3c-1.3-5.7-6-7.7-10-7.7c-3.4 0-6.7 1.4-8.9 3.7c-1.9 2-2.9 4.7-2.7 7.7z" clip-path="url(#deviconGo1)" />
                                <path fill="#fff" fill-rule="evenodd" d="M37.5 24.5c3.2 12.3 22.9 9.2 22.2-3.2c-.9-14.8-25.3-12-22.2 3.2" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path d="M48 32.7c-4.3 0-9.3-2.1-10.9-8.1c-.7-3.5 0-6.7 2-9.1c2.2-2.7 5.8-4.3 9.7-4.3c5.2 0 10.7 3.1 11.1 10.1c.2 2.9-.7 5.5-2.7 7.6c-2.1 2.3-5.6 3.8-9.2 3.8zm.8-20.8c-3.7 0-7.1 1.5-9.2 4c-1.9 2.3-2.5 5.2-1.8 8.5C39.2 30 44 32 48 32c3.4 0 6.7-1.3 8.8-3.6c1.8-1.9 2.7-4.4 2.5-7.1c-.2-4.3-3.1-9.4-10.5-9.4z" clip-path="url(#deviconGo1)" />
                                <path fill="#fff" fill-rule="evenodd" d="M68 39.2c0 1.8.4 3.9.1 5.9c-.5.9-1.4 1-2.2 1.3c-1.1-.2-2-.9-2.5-1.9c-.3-2.2.1-4.4.2-6.6l4.4 1.3z" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path d="M65.9 46.8c-1.3-.2-2.3-1-2.8-2.1c-.2-1.6-.1-3.1 0-4.6c.1-.7.1-1.4.1-2.1v-.4l5.1 1.6v.2c0 .6.1 1.2.1 1.9c.1 1.3.2 2.7 0 4v.1c-.4.8-1.1 1-1.8 1.3c-.2-.1-.4 0-.7.1zm-2.2-2.4c.4.9 1.2 1.5 2.1 1.7c.2-.1.4-.1.5-.2c.6-.2 1.1-.4 1.4-.9c.2-1.2.1-2.5 0-3.8c0-.6-.1-1.2-.1-1.7l-3.8-1.2c0 .6-.1 1.2-.1 1.7c-.1 1.6-.2 3 0 4.4z" clip-path="url(#deviconGo1)" />
                                <path fill-rule="evenodd" d="M46.3 22.5c0 2-1.5 3.6-3.3 3.6c-1.8 0-3.3-1.6-3.3-3.6s1.5-3.6 3.3-3.6c1.8 0 3.3 1.6 3.3 3.6" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path fill="#fff" fill-rule="evenodd" d="M45.2 23.3c0 .5-.4.9-.8.9s-.8-.4-.8-.9s.4-.9.8-.9c.5 0 .8.4.8.9" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path fill-rule="evenodd" d="M74.2 21.6c0 2-1.5 3.6-3.3 3.6c-1.8 0-3.3-1.6-3.3-3.6s1.5-3.6 3.3-3.6c1.8 0 3.3 1.6 3.3 3.6" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path fill="#fff" fill-rule="evenodd" d="M73.2 22.4c0 .5-.3.9-.8.9c-.4 0-.8-.4-.8-.9s.3-.9.8-.9c.4 0 .8.4.8.9M58.4 39c-1.5 3.5.8 10.6 4.8 5.4c-.3-2.2.1-4.4.2-6.6l-5 1.2z" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path d="M60.5 46.6c-.7 0-1.4-.4-1.9-1.2c-1.1-1.6-1.3-4.6-.5-6.5l.1-.2l5.5-1.4v.4l-.1 2.2c-.1 1.5-.2 2.9 0 4.4v.1l-.1.1c-1 1.4-2 2.1-3 2.1zm-1.8-7.3c-.6 1.7-.4 4.4.5 5.7c.4.6.8.9 1.3.9c.7 0 1.5-.6 2.3-1.6c-.2-1.5-.1-3 .1-4.4l.1-1.7l-4.3 1.1z" clip-path="url(#deviconGo1)" />
                                <path fill="#F6D2A2" fill-rule="evenodd" d="M58.9 32.2c-2.7.2-4.9 3.5-3.5 6c1.9 3.4 6-.3 8.6 0c3 .1 5.4 3.2 7.8.6c2.7-2.9-1.2-5.7-4.1-7l-8.8.4z" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                                <path fill="#231F20" d="M69.7 40.2c-.9 0-1.8-.4-2.7-.8c-.9-.4-1.9-.8-3-.8h-.3c-.8 0-1.7.3-2.7.7c-1.1.4-2.2.7-3.2.7c-1.2 0-2.1-.5-2.7-1.6c-.7-1.2-.6-2.6.1-3.9c.8-1.5 2.2-2.4 3.7-2.6l8.9-.4h.1c2.2.9 4.7 2.6 5.2 4.6c.2 1-.1 2-.9 2.9c-.8.9-1.6 1.2-2.5 1.2zM64.1 38c1.1 0 2.2.5 3.2.9c.9.4 1.7.7 2.5.7c.7 0 1.3-.3 1.9-.9c.7-.7.9-1.5.8-2.3c-.4-1.7-2.8-3.3-4.7-4.1l-8.7.4c-1.3.1-2.5 1-3.2 2.2c-.6 1.1-.6 2.3-.1 3.3c.5.9 1.1 1.3 2.1 1.3c.9 0 1.9-.4 2.9-.7c1.1-.4 2-.7 3-.7c0-.2.1-.2.3-.1z" clip-path="url(#deviconGo1)" />
                                <path fill-rule="evenodd" d="M58.6 32.1c-.2-4.7 8.8-5.3 9.8-1.4c1.1 4-9.4 4.9-9.8 1.4" clip-path="url(#deviconGo1)" clip-rule="evenodd" />
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-xl">Go</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="45"
                                height="45"
                                viewBox="0 0 256 346"
                                className="flex-shrink-0"
                            >
                                <path fill="#5382A1" d="M82.554 267.473s-13.198 7.675 9.393 10.272c27.369 3.122 41.356 2.675 71.517-3.034c0 0 7.93 4.972 19.003 9.279c-67.611 28.977-153.019-1.679-99.913-16.517m-8.262-37.814s-14.803 10.958 7.805 13.296c29.236 3.016 52.324 3.263 92.276-4.43c0 0 5.526 5.602 14.215 8.666c-81.747 23.904-172.798 1.885-114.296-17.532" />
                                <path fill="#E76F00" d="M143.942 165.515c16.66 19.18-4.377 36.44-4.377 36.44s42.301-21.837 22.874-49.183c-18.144-25.5-32.059-38.172 43.268-81.858c0 0-118.238 29.53-61.765 94.6" />
                                <path fill="#5382A1" d="M233.364 295.442s9.767 8.047-10.757 14.273c-39.026 11.823-162.432 15.393-196.714.471c-12.323-5.36 10.787-12.8 18.056-14.362c7.581-1.644 11.914-1.337 11.914-1.337c-13.705-9.655-88.583 18.957-38.034 27.15c137.853 22.356 251.292-10.066 215.535-26.195M88.9 190.48s-62.771 14.91-22.228 20.323c17.118 2.292 51.243 1.774 83.03-.89c25.978-2.19 52.063-6.85 52.063-6.85s-9.16 3.923-15.787 8.448c-63.744 16.765-186.886 8.966-151.435-8.183c29.981-14.492 54.358-12.848 54.358-12.848m112.605 62.942c64.8-33.672 34.839-66.03 13.927-61.67c-5.126 1.066-7.411 1.99-7.411 1.99s1.903-2.98 5.537-4.27c41.37-14.545 73.187 42.897-13.355 65.647c0 .001 1.003-.895 1.302-1.697" />
                                <path fill="#E76F00" d="M162.439.371s35.887 35.9-34.037 91.101c-56.071 44.282-12.786 69.53-.023 98.377c-32.73-29.53-56.75-55.526-40.635-79.72C111.395 74.612 176.918 57.393 162.439.37" />
                                <path fill="#5382A1" d="M95.268 344.665c62.199 3.982 157.712-2.209 159.974-31.64c0 0-4.348 11.158-51.404 20.018c-53.088 9.99-118.564 8.824-157.399 2.421c.001 0 7.95 6.58 48.83 9.201" />
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-xl">Java</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="45"
                                height="45"
                                viewBox="0 0 24 24"
                                className="flex-shrink-0"
                            >
                                <path fill="green" d="m23.693 10.706l-4.73-8.185c-.41-.71-1.417-1.294-2.24-1.294h-9.45c-.82 0-1.831.584-2.24 1.294L.306 10.706c-.41.71-.41 1.873 0 2.584l4.725 8.189c.41.71 1.417 1.294 2.24 1.294h9.455c.82 0 1.826-.584 2.24-1.294l4.726-8.19c.41-.71.41-1.873 0-2.583zM10.976 5.755c0-.537.438-.975.974-.975s.975.438.975.975v5.821a.976.976 0 0 1-1.948 0zm.974 12.43a6.616 6.616 0 0 1-6.607-6.609A6.638 6.638 0 0 1 8.01 6.272a.866.866 0 0 1 1.214.18a.866.866 0 0 1-.178 1.213a4.876 4.876 0 0 0 5.812 7.827a4.875 4.875 0 0 0 1.967-3.916a4.897 4.897 0 0 0-1.986-3.925a.87.87 0 0 1-.183-1.214a.87.87 0 0 1 1.214-.183a6.631 6.631 0 0 1 2.687 5.322a6.613 6.613 0 0 1-6.608 6.608z" />
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-lg">SpringBoot</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="45"
                                height="45"
                                viewBox="0 0 256 256"
                                className="flex-shrink-0"
                            >
                                <g fill="none">
                                    <rect width="256" height="256" fill="#007ACC" rx="60" />
                                    <path fill="#fff" d="m56.611 128.849l-.081 10.484h33.32v94.679h23.57v-94.679h33.32v-10.281c0-5.689-.121-10.443-.284-10.565c-.122-.162-20.399-.244-44.983-.203l-44.739.122l-.122 10.443Zm149.956-10.741c6.501 1.626 11.459 4.511 16.01 9.224c2.357 2.52 5.851 7.112 6.136 8.209c.081.325-11.053 7.802-17.798 11.987c-.244.163-1.22-.894-2.317-2.519c-3.291-4.795-6.745-6.868-12.028-7.233c-7.761-.529-12.759 3.535-12.718 10.321c0 1.991.284 3.169 1.097 4.795c1.706 3.535 4.876 5.648 14.832 9.955c18.326 7.884 26.168 13.085 31.045 20.48c5.445 8.249 6.664 21.415 2.966 31.208c-4.063 10.646-14.141 17.879-28.323 20.277c-4.388.772-14.791.65-19.504-.203c-10.281-1.829-20.033-6.908-26.047-13.572c-2.357-2.601-6.949-9.387-6.664-9.875c.121-.162 1.178-.812 2.356-1.503c1.138-.65 5.446-3.129 9.509-5.486l7.355-4.267l1.544 2.276c2.154 3.291 6.867 7.802 9.712 9.305c8.167 4.308 19.383 3.698 24.909-1.259c2.357-2.154 3.332-4.389 3.332-7.68c0-2.967-.366-4.267-1.91-6.502c-1.991-2.844-6.054-5.242-17.595-10.24c-13.206-5.689-18.895-9.224-24.096-14.832c-3.007-3.25-5.852-8.452-7.03-12.8c-.975-3.616-1.219-12.678-.447-16.335c2.722-12.759 12.353-21.658 26.25-24.3c4.511-.853 14.994-.528 19.424.569Z" />
                                </g>
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-lg">TypeScript</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="45"
                                height="45"
                                viewBox="0 0 256 228"
                                className="flex-shrink-0"
                            >
                                <path fill="#00D8FF" d="M210.483 73.824a171.49 171.49 0 0 0-8.24-2.597c.465-1.9.893-3.777 1.273-5.621c6.238-30.281 2.16-54.676-11.769-62.708c-13.355-7.7-35.196.329-57.254 19.526a171.23 171.23 0 0 0-6.375 5.848a155.866 155.866 0 0 0-4.241-3.917C100.759 3.829 77.587-4.822 63.673 3.233C50.33 10.957 46.379 33.89 51.995 62.588a170.974 170.974 0 0 0 1.892 8.48c-3.28.932-6.445 1.924-9.474 2.98C17.309 83.498 0 98.307 0 113.668c0 15.865 18.582 31.778 46.812 41.427a145.52 145.52 0 0 0 6.921 2.165a167.467 167.467 0 0 0-2.01 9.138c-5.354 28.2-1.173 50.591 12.134 58.266c13.744 7.926 36.812-.22 59.273-19.855a145.567 145.567 0 0 0 5.342-4.923a168.064 168.064 0 0 0 6.92 6.314c21.758 18.722 43.246 26.282 56.54 18.586c13.731-7.949 18.194-32.003 12.4-61.268a145.016 145.016 0 0 0-1.535-6.842c1.62-.48 3.21-.974 4.76-1.488c29.348-9.723 48.443-25.443 48.443-41.52c0-15.417-17.868-30.326-45.517-39.844Zm-6.365 70.984c-1.4.463-2.836.91-4.3 1.345c-3.24-10.257-7.612-21.163-12.963-32.432c5.106-11 9.31-21.767 12.459-31.957c2.619.758 5.16 1.557 7.61 2.4c23.69 8.156 38.14 20.213 38.14 29.504c0 9.896-15.606 22.743-40.946 31.14Zm-10.514 20.834c2.562 12.94 2.927 24.64 1.23 33.787c-1.524 8.219-4.59 13.698-8.382 15.893c-8.067 4.67-25.32-1.4-43.927-17.412a156.726 156.726 0 0 1-6.437-5.87c7.214-7.889 14.423-17.06 21.459-27.246c12.376-1.098 24.068-2.894 34.671-5.345a134.17 134.17 0 0 1 1.386 6.193ZM87.276 214.515c-7.882 2.783-14.16 2.863-17.955.675c-8.075-4.657-11.432-22.636-6.853-46.752a156.923 156.923 0 0 1 1.869-8.499c10.486 2.32 22.093 3.988 34.498 4.994c7.084 9.967 14.501 19.128 21.976 27.15a134.668 134.668 0 0 1-4.877 4.492c-9.933 8.682-19.886 14.842-28.658 17.94ZM50.35 144.747c-12.483-4.267-22.792-9.812-29.858-15.863c-6.35-5.437-9.555-10.836-9.555-15.216c0-9.322 13.897-21.212 37.076-29.293c2.813-.98 5.757-1.905 8.812-2.773c3.204 10.42 7.406 21.315 12.477 32.332c-5.137 11.18-9.399 22.249-12.634 32.792a134.718 134.718 0 0 1-6.318-1.979Zm12.378-84.26c-4.811-24.587-1.616-43.134 6.425-47.789c8.564-4.958 27.502 2.111 47.463 19.835a144.318 144.318 0 0 1 3.841 3.545c-7.438 7.987-14.787 17.08-21.808 26.988c-12.04 1.116-23.565 2.908-34.161 5.309a160.342 160.342 0 0 1-1.76-7.887Zm110.427 27.268a347.8 347.8 0 0 0-7.785-12.803c8.168 1.033 15.994 2.404 23.343 4.08c-2.206 7.072-4.956 14.465-8.193 22.045a381.151 381.151 0 0 0-7.365-13.322Zm-45.032-43.861c5.044 5.465 10.096 11.566 15.065 18.186a322.04 322.04 0 0 0-30.257-.006c4.974-6.559 10.069-12.652 15.192-18.18ZM82.802 87.83a323.167 323.167 0 0 0-7.227 13.238c-3.184-7.553-5.909-14.98-8.134-22.152c7.304-1.634 15.093-2.97 23.209-3.984a321.524 321.524 0 0 0-7.848 12.897Zm8.081 65.352c-8.385-.936-16.291-2.203-23.593-3.793c2.26-7.3 5.045-14.885 8.298-22.6a321.187 321.187 0 0 0 7.257 13.246c2.594 4.48 5.28 8.868 8.038 13.147Zm37.542 31.03c-5.184-5.592-10.354-11.779-15.403-18.433c4.902.192 9.899.29 14.978.29c5.218 0 10.376-.117 15.453-.343c-4.985 6.774-10.018 12.97-15.028 18.486Zm52.198-57.817c3.422 7.8 6.306 15.345 8.596 22.52c-7.422 1.694-15.436 3.058-23.88 4.071a382.417 382.417 0 0 0 7.859-13.026a347.403 347.403 0 0 0 7.425-13.565Zm-16.898 8.101a358.557 358.557 0 0 1-12.281 19.815a329.4 329.4 0 0 1-23.444.823c-7.967 0-15.716-.248-23.178-.732a310.202 310.202 0 0 1-12.513-19.846h.001a307.41 307.41 0 0 1-10.923-20.627a310.278 310.278 0 0 1 10.89-20.637l-.001.001a307.318 307.318 0 0 1 12.413-19.761c7.613-.576 15.42-.876 23.31-.876H128c7.926 0 15.743.303 23.354.883a329.357 329.357 0 0 1 12.335 19.695a358.489 358.489 0 0 1 11.036 20.54a329.472 329.472 0 0 1-11 20.722Zm22.56-122.124c8.572 4.944 11.906 24.881 6.52 51.026c-.344 1.668-.73 3.367-1.15 5.09c-10.622-2.452-22.155-4.275-34.23-5.408c-7.034-10.017-14.323-19.124-21.64-27.008a160.789 160.789 0 0 1 5.888-5.4c18.9-16.447 36.564-22.941 44.612-18.3ZM128 90.808c12.625 0 22.86 10.235 22.86 22.86s-10.235 22.86-22.86 22.86s-22.86-10.235-22.86-22.86s10.235-22.86 22.86-22.86Z" />
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-xl">React</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="45"
                                height="45"
                                viewBox="0 0 256 256"
                                className="flex-shrink-0"
                            >
                                <defs>
                                    <path id="skillIconsRemixDark0" fill="#fff" fill-rule="evenodd" d="M187.903 168.726c1.361 17.488 1.361 25.686 1.361 34.634h-40.462c0-1.949.035-3.732.07-5.54c.11-5.621.224-11.482-.687-23.318c-1.203-17.329-8.665-21.179-22.386-21.179H62.16v-31.528h65.565c17.331 0 25.997-5.272 25.997-19.231c0-12.275-8.666-19.713-25.997-19.713H62.16V52h72.786c39.236 0 58.734 18.532 58.734 48.134c0 22.142-13.72 36.582-32.256 38.989c15.647 3.129 24.794 12.034 26.479 29.603Z" clip-rule="evenodd" />
                                </defs>
                                <g fill="none">
                                    <rect width="256" height="256" fill="#242938" rx="60" />
                                    <use href="#skillIconsRemixDark0" fill-rule="evenodd" clip-rule="evenodd" />
                                    <use href="#skillIconsRemixDark0" fill-rule="evenodd" clip-rule="evenodd" />
                                    <path fill="#fff" d="M62.16 203.36v-23.503h42.783c7.146 0 8.698 5.3 8.698 8.461v15.042h-51.48Z" />
                                    <path stroke="#fff" stroke-opacity=".8" stroke-width=".32" d="M62 203.36v.16h51.801v-15.202c0-1.602-.392-3.755-1.701-5.512c-1.314-1.765-3.539-3.109-7.157-3.109H62v23.663Z" />
                                </g>
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-xl">Remix</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="45"
                                height="45"
                                viewBox="0 0 256 256"
                                className="flex-shrink-0"
                            >
                                <defs>
                                    <linearGradient id="logosNextjsIcon0" x1="55.633%" x2="83.228%" y1="56.385%" y2="96.08%">
                                        <stop offset="0%" stop-color="#FFF" /><stop offset="100%" stop-color="#FFF" stop-opacity="0" />
                                    </linearGradient>
                                    <linearGradient id="logosNextjsIcon1" x1="50%" x2="49.953%" y1="0%" y2="73.438%">
                                        <stop offset="0%" stop-color="#FFF" /><stop offset="100%" stop-color="#FFF" stop-opacity="0" />
                                    </linearGradient><circle id="logosNextjsIcon2" cx="128" cy="128" r="128" />
                                </defs>
                                <mask id="logosNextjsIcon3" fill="#fff">
                                    <use href="#logosNextjsIcon2" />
                                </mask>
                                <g mask="url(#logosNextjsIcon3)">
                                    <circle cx="128" cy="128" r="128" />
                                    <path fill="url(#logosNextjsIcon0)" d="M212.634 224.028L98.335 76.8H76.8v102.357h17.228V98.68L199.11 234.446a128.433 128.433 0 0 0 13.524-10.418Z" />
                                    <path fill="url(#logosNextjsIcon1)" d="M163.556 76.8h17.067v102.4h-17.067z" />
                                </g>
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-xl">Next.js</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="45"
                                height="45"
                                viewBox="0 0 256 153"
                                className="flex-shrink-0"
                            >
                                <path fill="#252F3E" d="M72.392 55.438c0 3.137.34 5.68.933 7.545a45.373 45.373 0 0 0 2.712 6.103c.424.678.593 1.356.593 1.95c0 .847-.508 1.695-1.61 2.543l-5.34 3.56c-.763.509-1.526.763-2.205.763c-.847 0-1.695-.424-2.543-1.187a26.224 26.224 0 0 1-3.051-3.984c-.848-1.44-1.696-3.052-2.628-5.001c-6.612 7.798-14.92 11.698-24.922 11.698c-7.12 0-12.8-2.035-16.954-6.103c-4.153-4.07-6.272-9.495-6.272-16.276c0-7.205 2.543-13.054 7.714-17.462c5.17-4.408 12.037-6.612 20.768-6.612c2.882 0 5.849.254 8.985.678c3.137.424 6.358 1.102 9.749 1.865V29.33c0-6.443-1.357-10.935-3.985-13.563c-2.712-2.628-7.29-3.9-13.817-3.9c-2.967 0-6.018.34-9.155 1.103c-3.136.762-6.188 1.695-9.155 2.882c-1.356.593-2.373.932-2.967 1.102c-.593.17-1.017.254-1.356.254c-1.187 0-1.78-.848-1.78-2.628v-4.154c0-1.356.17-2.373.593-2.966c.424-.594 1.187-1.187 2.374-1.78c2.967-1.526 6.527-2.798 10.68-3.815C33.908.763 38.316.255 42.978.255c10.088 0 17.463 2.288 22.21 6.866c4.662 4.577 7.036 11.528 7.036 20.853v27.464h.17ZM37.976 68.323c2.798 0 5.68-.508 8.731-1.526c3.052-1.017 5.765-2.882 8.053-5.425c1.357-1.61 2.374-3.39 2.882-5.425c.509-2.034.848-4.493.848-7.375v-3.56a70.791 70.791 0 0 0-7.799-1.441a63.874 63.874 0 0 0-7.968-.509c-5.68 0-9.833 1.102-12.63 3.391c-2.798 2.289-4.154 5.51-4.154 9.748c0 3.984 1.017 6.951 3.136 8.986c2.035 2.119 5.002 3.136 8.901 3.136Zm68.069 9.155c-1.526 0-2.543-.254-3.221-.848c-.678-.508-1.272-1.695-1.78-3.305L81.124 7.799c-.51-1.696-.764-2.798-.764-3.391c0-1.356.678-2.12 2.035-2.12h8.307c1.61 0 2.713.255 3.306.848c.678.509 1.187 1.696 1.695 3.306l14.241 56.117l13.224-56.117c.424-1.695.933-2.797 1.61-3.306c.679-.508 1.866-.847 3.392-.847h6.781c1.61 0 2.713.254 3.39.847c.679.509 1.272 1.696 1.611 3.306l13.394 56.795L168.01 6.442c.508-1.695 1.102-2.797 1.695-3.306c.678-.508 1.78-.847 3.306-.847h7.883c1.357 0 2.12.678 2.12 2.119c0 .424-.085.848-.17 1.356c-.085.509-.254 1.187-.593 2.12l-20.43 65.525c-.508 1.696-1.101 2.798-1.78 3.306c-.678.509-1.78.848-3.22.848h-7.29c-1.611 0-2.713-.254-3.392-.848c-.678-.593-1.271-1.695-1.61-3.39l-13.14-54.676l-13.054 54.59c-.423 1.696-.932 2.798-1.61 3.391c-.678.594-1.865.848-3.39.848h-7.291Zm108.927 2.289c-4.408 0-8.816-.509-13.054-1.526c-4.239-1.017-7.544-2.12-9.748-3.39c-1.357-.764-2.29-1.611-2.628-2.374a5.983 5.983 0 0 1-.509-2.374V65.78c0-1.78.678-2.628 1.95-2.628a4.8 4.8 0 0 1 1.526.255c.508.17 1.271.508 2.119.847a46.108 46.108 0 0 0 9.324 2.967a50.907 50.907 0 0 0 10.088 1.017c5.34 0 9.494-.932 12.376-2.797c2.882-1.865 4.408-4.577 4.408-8.053c0-2.373-.763-4.323-2.289-5.934c-1.526-1.61-4.408-3.051-8.561-4.408l-12.292-3.814c-6.188-1.95-10.765-4.832-13.563-8.647c-2.797-3.73-4.238-7.883-4.238-12.291c0-3.56.763-6.697 2.289-9.41c1.525-2.712 3.56-5.085 6.103-6.95c2.543-1.95 5.425-3.391 8.816-4.408c3.39-1.017 6.95-1.441 10.68-1.441c1.865 0 3.815.085 5.68.339c1.95.254 3.73.593 5.51.932c1.695.424 3.306.848 4.832 1.357c1.526.508 2.712 1.017 3.56 1.525c1.187.679 2.034 1.357 2.543 2.12c.509.678.763 1.61.763 2.797v3.984c0 1.78-.678 2.713-1.95 2.713c-.678 0-1.78-.34-3.22-1.018c-4.833-2.204-10.258-3.306-16.276-3.306c-4.832 0-8.647.763-11.275 2.374c-2.627 1.61-3.984 4.069-3.984 7.544c0 2.374.848 4.408 2.543 6.019c1.696 1.61 4.832 3.221 9.325 4.662l12.037 3.815c6.103 1.95 10.511 4.662 13.139 8.137c2.628 3.476 3.9 7.46 3.9 11.868c0 3.645-.764 6.951-2.205 9.833c-1.525 2.882-3.56 5.425-6.188 7.46c-2.628 2.119-5.764 3.645-9.409 4.747c-3.815 1.187-7.799 1.78-12.122 1.78Z" />
                                <path fill="#F90" d="M230.993 120.964c-27.888 20.599-68.408 31.534-103.247 31.534c-48.827 0-92.821-18.056-126.05-48.064c-2.628-2.373-.255-5.594 2.881-3.73c35.942 20.854 80.276 33.484 126.136 33.484c30.94 0 64.932-6.442 96.212-19.666c4.662-2.12 8.646 3.052 4.068 6.442Zm11.614-13.224c-3.56-4.577-23.566-2.204-32.636-1.102c-2.713.34-3.137-2.034-.678-3.814c15.936-11.19 42.13-7.968 45.181-4.239c3.052 3.815-.848 30.008-15.767 42.554c-2.288 1.95-4.492.933-3.475-1.61c3.39-8.393 10.935-27.296 7.375-31.789Z" />
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-lg">AWS</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="50"
                                height="50"
                                viewBox="0 0 128 128"
                                className="flex-shrink-0"
                            >
                                <path fill="#3A4D54" fill-rule="evenodd" d="M73.8 50.8h11.3v11.5h5.7c2.6 0 5.3-.5 7.8-1.3c1.2-.4 2.6-1 3.8-1.7c-1.6-2.1-2.4-4.7-2.6-7.3c-.3-3.5.4-8.1 2.8-10.8l1.2-1.4l1.4 1.1c3.6 2.9 6.5 6.8 7.1 11.4c4.3-1.3 9.3-1 13.1 1.2l1.5.9l-.8 1.6c-3.2 6.2-9.9 8.2-16.4 7.8c-9.8 24.3-31 35.8-56.8 35.8c-13.3 0-25.5-5-32.5-16.8l-.1-.2l-1-2.1c-2.4-5.2-3.1-10.9-2.6-16.6l.2-1.7h9.6V50.8h11.3V39.6h22.5V28.3h13.5v22.5z" clip-rule="evenodd" />
                                <path fill="#00AADA" d="M110.4 55.1c.8-5.9-3.6-10.5-6.4-12.7c-3.1 3.6-3.6 13.2 1.3 17.2c-2.8 2.4-8.5 4.7-14.5 4.7H18.6c-.6 6.2.5 11.9 3 16.8l.8 1.5c.5.9 1.1 1.7 1.7 2.6c3 .2 5.7.3 8.2.2c4.9-.1 8.9-.7 12-1.7c.5-.2.9.1 1.1.5c.2.5-.1.9-.5 1.1c-.4.1-.8.3-1.3.4c-2.4.7-5 1.1-8.3 1.3h-.6c-1.3.1-2.7.1-4.2.1c-1.6 0-3.1 0-4.9-.1c6 6.8 15.4 10.8 27.2 10.8c25 0 46.2-11.1 55.5-35.9c6.7.7 13.1-1 16-6.7c-4.5-2.7-10.5-1.8-13.9-.1z" />
                                <path fill="#28B8EB" d="M110.4 55.1c.8-5.9-3.6-10.5-6.4-12.7c-3.1 3.6-3.6 13.2 1.3 17.2c-2.8 2.4-8.5 4.7-14.5 4.7h-68c-.3 9.5 3.2 16.7 9.5 21c4.9-.1 8.9-.7 12-1.7c.5-.2.9.1 1.1.5c.2.5-.1.9-.5 1.1c-.4.1-.8.3-1.3.4c-2.4.7-5.2 1.2-8.5 1.4l-.1-.1c8.5 4.4 20.8 4.3 35-1.1c15.8-6.1 30.6-17.7 40.9-30.9c-.2.1-.4.1-.5.2z" />
                                <path fill="#028BB8" d="M18.7 71.8c.4 3.3 1.4 6.4 2.9 9.3l.8 1.5c.5.9 1.1 1.7 1.7 2.6c3 .2 5.7.3 8.2.2c4.9-.1 8.9-.7 12-1.7c.5-.2.9.1 1.1.5c.2.5-.1.9-.5 1.1c-.4.1-.8.3-1.3.4c-2.4.7-5.2 1.2-8.5 1.4h-.4c-1.3.1-2.7.1-4.1.1c-1.6 0-3.2 0-4.9-.1c6 6.8 15.5 10.8 27.3 10.8c21.4 0 40-8.1 50.8-26H18.7v-.1z" />
                                <path fill="#019BC6" d="M23.5 71.8c1.3 5.8 4.3 10.4 8.8 13.5c4.9-.1 8.9-.7 12-1.7c.5-.2.9.1 1.1.5c.2.5-.1.9-.5 1.1c-.4.1-.8.3-1.3.4c-2.4.7-5.2 1.2-8.6 1.4c8.5 4.4 20.8 4.3 34.9-1.1c8.5-3.3 16.8-8.2 24.2-14.1H23.5z" />
                                <path fill="#00ACD3" fill-rule="evenodd" d="M28.4 52.7h9.8v9.8h-9.8v-9.8zm.8.8h.8v8.1h-.8v-8.1zm1.4 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm3-12h9.8v9.8h-9.8v-9.8zm.9.8h.8v8.1h-.8v-8.1zm1.4 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.4 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1z" clip-rule="evenodd" />
                                <path fill="#23C2EE" fill-rule="evenodd" d="M39.6 52.7h9.8v9.8h-9.8v-9.8zm.9.8h.8v8.1h-.8v-8.1zm1.4 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.4 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1z" clip-rule="evenodd" />
                                <path fill="#00ACD3" fill-rule="evenodd" d="M50.9 52.7h9.8v9.8h-9.8v-9.8zm.8.8h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.4 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1z" clip-rule="evenodd" />
                                <path fill="#23C2EE" fill-rule="evenodd" d="M50.9 41.5h9.8v9.8h-9.8v-9.8zm.8.8h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.4 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm3.1 10.4H72v9.8h-9.8v-9.8zm.8.8h.8v8.1H63v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.4 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1z" clip-rule="evenodd" />
                                <path fill="#00ACD3" fill-rule="evenodd" d="M62.2 41.5H72v9.8h-9.8v-9.8zm.8.8h.8v8.1H63v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.4 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1z" clip-rule="evenodd" />
                                <path fill="#23C2EE" fill-rule="evenodd" d="M62.2 30.2H72V40h-9.8v-9.8zm.8.8h.8v8.1H63V31zm1.5 0h.8v8.1h-.8V31zm1.4 0h.8v8.1h-.8V31zm1.5 0h.8v8.1h-.8V31zm1.5 0h.8v8.1h-.8V31zm1.5 0h.8v8.1h-.8V31z" clip-rule="evenodd" />
                                <path fill="#00ACD3" fill-rule="evenodd" d="M73.5 52.7h9.8v9.8h-9.8v-9.8zm.8.8h.8v8.1h-.8v-8.1zm1.4 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1zm1.5 0h.8v8.1h-.8v-8.1z" clip-rule="evenodd" />
                                <path fill="#D4EEF1" fill-rule="evenodd" d="M48.8 78.3c1.5 0 2.7 1.2 2.7 2.7c0 1.5-1.2 2.7-2.7 2.7c-1.5 0-2.7-1.2-2.7-2.7c0-1.5 1.2-2.7 2.7-2.7" clip-rule="evenodd" />
                                <path fill="#3A4D54" fill-rule="evenodd" d="M48.8 79.1c.2 0 .5 0 .7.1c-.2.1-.4.4-.4.7c0 .4.4.8.8.8c.3 0 .6-.2.7-.4c.1.2.1.5.1.7c0 1.1-.9 1.9-1.9 1.9c-1.1 0-1.9-.9-1.9-1.9c0-1 .8-1.9 1.9-1.9M1.1 72.8h125.4c-2.7-.7-8.6-1.6-7.7-5.2c-5 5.7-16.9 4-20 1.2c-3.4 4.9-23 3-24.3-.8c-4.2 5-17.3 5-21.5 0c-1.4 3.8-21 5.7-24.3.8c-3 2.8-15 4.5-20-1.2c1.1 3.5-4.9 4.5-7.6 5.2" clip-rule="evenodd" />
                                <path fill="#BFDBE0" d="M56 97.8c-6.7-3.2-10.3-7.5-12.4-12.2c-2.5.7-5.5 1.2-8.9 1.4c-1.3.1-2.7.1-4.1.1c-1.7 0-3.4 0-5.2-.1c6 6 13.6 10.7 27.5 10.8H56z" />
                                <path fill="#D4EEF1" d="M46.1 89.9c-.9-1.3-1.8-2.8-2.5-4.3c-2.5.7-5.5 1.2-8.9 1.4c2.3 1.2 5.7 2.4 11.4 2.9z" />
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-xl">Docker</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="45"
                                height="45"
                                viewBox="0 0 512 349"
                                className="flex-shrink-0"
                            >
                                <path fill="#00758F" d="m152.31 230.297l15.56 50.487c3.496 11.463 4.954 19.465 4.37 24.026c8.51-22.792 14.456-47.63 17.839-74.513h18.71c-8.045 43.766-18.656 75.57-31.827 95.41c-10.262 15.289-21.504 22.933-33.746 22.933c-3.264 0-7.288-.986-12.063-2.944v-10.55c2.333.342 5.07.525 8.218.525c5.711 0 10.314-1.583 13.816-4.742c4.193-3.849 6.292-8.175 6.292-12.97c0-3.274-1.637-9.993-4.896-20.157l-21.68-67.505h19.406ZM33.223 199.266l28.5 86.956h.176l28.675-86.956h23.428c5.13 43.124 8.16 82.581 9.09 118.346H103.34c-.695-33.433-2.62-64.871-5.768-94.32H97.4l-30.078 94.32H52.28l-29.896-94.32h-.176c-2.218 28.282-3.614 59.72-4.196 94.32H0c1.164-42.08 4.077-81.525 8.739-118.346h24.485Z" />
                                <path fill="#F29111" d="M352.498 197.51c30.657 0 45.986 19.586 45.986 58.739c0 21.276-4.61 37.347-13.821 48.204c-1.66 1.984-3.495 3.698-5.427 5.286l21.695 10.727l-.021-.001l-7.703 13.302l-28.253-16.485c-4.683 1.387-9.836 2.08-15.451 2.08c-15.053 0-26.297-4.387-33.731-13.15c-8.16-9.694-12.238-24.955-12.238-45.757c0-21.156 4.602-37.166 13.816-48.037c8.392-9.944 20.11-14.909 35.148-14.909Zm-93.88.172c10.957 0 20.92 2.932 29.894 8.775l-4.558 10.157c-7.679-3.264-15.25-4.903-22.716-4.903c-6.058 0-10.726 1.458-13.98 4.392c-3.272 2.908-5.296 6.65-5.296 11.212c0 7.01 4.994 13.089 14.215 18.225a816.32 816.32 0 0 1 9.031 5.011l.688.387l.345.194l.689.387l.344.194l.688.388c6.98 3.935 13.548 7.691 13.548 7.691c9.22 6.545 13.816 13.523 13.816 25.016c0 10.037-3.678 18.276-11.01 24.723c-7.337 6.418-17.194 9.636-29.538 9.636c-11.545 0-22.734-3.704-33.572-11.05l5.07-10.166c9.327 4.675 17.767 7.01 25.346 7.01c7.108 0 12.672-1.587 16.697-4.721c4.017-3.157 6.424-7.56 6.424-13.143c0-7.027-4.888-13.034-13.855-18.073a897.982 897.982 0 0 1-8.395-4.697l-.687-.389c-1.262-.713-2.533-1.435-3.778-2.142l-.675-.384c-6.055-3.444-11.29-6.453-11.29-6.453c-8.964-6.557-13.459-13.592-13.459-25.184c0-9.587 3.352-17.336 10.046-23.231c6.71-5.908 15.367-8.862 25.968-8.862Zm175.895 1.584v103.788h37.238v14.558h-56.124V199.266h18.886Zm57.93 103.833v2.46h-4.094v12.04h-3.13v-12.04h-4.253v-2.46h11.478Zm7.56 0l3.931 9.884l3.611-9.884h4.437v14.5h-2.95v-11.035l-4.11 11.035h-2.127l-4.117-11.035h-.158v11.035h-2.791v-14.5h4.275ZM350.57 212.064c-18.066 0-27.104 14.91-27.104 44.71c0 17.07 2.395 29.448 7.176 37.163c4.428 7.14 11.363 10.703 20.806 10.703c18.066 0 27.103-15.026 27.103-45.064c0-16.831-2.395-29.103-7.17-36.822c-4.433-7.124-11.365-10.69-20.81-10.69Z" />
                                <path fill="#00758F" d="M303.218 7.333c5.993-14.726 26.948-3.574 35.08 1.57c1.993 1.287 4.279 4.006 6.564 5.011c3.565.14 7.127.419 10.698.568c6.698 1.574 12.972 2.86 18.25 5.866c24.528 14.445 40.495 29.165 55.19 53.479c3.14 5.15 4.709 10.723 7.274 16.296c3.56 8.307 7.56 17.027 11.692 24.882c1.85 3.724 3.281 7.865 5.85 11.01c1.003 1.438 3.852 1.862 5.555 2.721c4.708 2.437 10.412 4.287 14.84 7.147c8.269 5.156 16.264 11.3 23.532 17.59c2.709 2.428 4.555 5.865 7.136 8.433v1.296c-2.291.703-4.574 1.423-6.859 2c-4.991 1.282-9.412.992-14.254 2.275c-2.992.868-6.707 2.013-9.845 2.304l.29.292c1.846 5.275 11.834 9.565 16.402 12.72c5.548 4.004 10.689 8.86 14.827 14.437c1.429 1.423 2.858 2.718 4.28 4.137c.994 1.438 1.274 3.298 2.28 4.58v.434c-1.114-.393-1.915-1.143-2.674-1.927l-.453-.473c-.453-.47-.91-.932-1.431-1.313c-3.148-2.15-6.274-4.722-9.422-6.721c-5.412-3.434-11.689-5.427-17.246-8.874c-3.142-2.001-6.137-4.28-9.132-6.57c-2.715-2.007-5.705-5.861-7.411-8.721c-1.005-1.58-1.143-3.437-2.291-4.58c.205-1.909 1.954-2.476 3.719-2.942l.406-.107c.609-.158 1.205-.316 1.725-.525c7.414-3.148 16.253-4.29 27.667-4.004c-.43-2.866-7.562-6.437-9.839-8.153c-4.57-3.294-9.409-6.731-14.257-9.729c-2.569-1.57-6.996-2.716-9.842-3.999c-3.851-1.574-12.41-3.147-14.544-6.145c-3.625-4.726-6.229-10.363-8.757-16.057l-.688-1.554a803.85 803.85 0 0 0-.69-1.553c-2.988-6.857-6.7-14.006-9.695-21.027c-1.566-3.425-2.285-6.431-4-9.716c-10.407-20.158-25.81-37.035-44.485-48.904c-6.137-3.862-12.98-7.436-20.534-9.865c-4.281-1.293-9.419-.578-13.98-1.57h-3.002c-2.562-.722-4.701-3.438-6.7-4.87c-4.415-2.998-8.837-5.011-14.117-7.15c-1.85-.858-7.133-2.856-8.977-1.283c-1.142.287-1.721.718-2.002 1.864c-1.136 1.71-.137 4.286.57 5.863c2.142 4.57 5.134 7.286 7.85 11.148c2.416 3.425 5.417 7.287 7.13 11.011c3.696 8.005 5.417 16.874 8.842 24.878c1.27 3.01 3.279 6.435 5.128 9.15c1.567 2.155 4.416 3.713 5.278 6.441c1.718 2.86-2.572 12.297-3.565 15.294c-3.715 11.727-2.995 28.028 1.283 38.193l.228.536l.228.543c1.562 3.723 3.234 7.732 7.387 8.773c.286-.284 0-.135.567-.284c1.005-7.868 1.288-15.445 4-21.601c1.567-3.849 4.696-6.57 6.841-9.712c1.43.856 1.43 3.437 2.282 5.145c1.856 4.43 3.849 9.287 6.137 13.73c4.696 9.15 9.98 18.021 15.967 26.025c2.005 2.859 4.85 6.006 7.416 8.581c1.143.997 2.423 1.573 3.282 2.856h.28v.432c-4.278-1.577-6.99-6.003-10.402-8.587c-6.424-4.857-14.117-12.151-18.545-19.15c-1.852-4.018-3.854-7.869-5.85-11.867v-.289c-.853 1.142-.567 2.276-.994 4.004c-1.852 7.145-.426 15.296-6.843 17.866c-7.274 3.01-12.7-4.857-14.977-8.432c-7.276-11.866-9.269-31.884-4.138-48.043c1.14-3.577 1.295-7.867 3.285-10.723c-.43-2.582-2.42-3.288-3.571-4.87c-1.996-2.704-3.705-5.854-5.268-8.857c-3.002-5.866-5.138-12.875-7.417-19.166c-1.002-2.569-1.289-5.148-2.288-7.58c-1.704-3.712-4.845-7.436-7.268-10.72c-3.281-4.72-12.837-13.868-8.985-23.168Zm46.772 28.015c.381.382.841.716 1.317 1.045l.574.394c.765.53 1.506 1.088 1.96 1.848c.72 1.006.854 1.999 1.716 3.007c0 3.437-.996 5.722-3.007 7.146c0 0-.137.15-.278.29c-1.14-2.291-2.139-4.57-3.287-6.859c-1.414-1.998-3.413-3.583-4.565-5.866h-.277v-.287c1.721-.425 3.428-.718 5.847-.718Z" />
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-lg">MySQL</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="45"
                                height="45"
                                viewBox="0 0 128 128"
                                className="flex-shrink-0"
                            >
                                <g fill-rule="evenodd">
                                    <path fill="#5c4ee5" d="M77.941 44.5v36.836L46.324 62.918V26.082zm0 0" />
                                    <path fill="#4040b2" d="m81.41 81.336l31.633-18.418V26.082L81.41 44.5zm0 0" />
                                    <path fill="#5c4ee5" d="M11.242 42.36L42.86 60.776V23.941L11.242 5.523zm66.699 43.015L46.324 66.957v36.82l31.617 18.418zm0 0" />
                                </g>
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-lg">Terraform</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                    <div className="mx-auto bg-background border p-2 rounded-lg">
                        {/* INFO: pタグで詳細文を記述する場合は下記のcssを適用する */}
                        {/* <div className="flex flex-col justify-between p-6 md:h-[160px] h-[180px]"> */}
                        <div className="w-[90px] h-[90px] flex flex-col items-center justify-center p-4">
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="45"
                                height="45"
                                viewBox="0 0 432 416"
                                className="flex-shrink-0"
                            >
                                <path fill="currentColor" d="M213.5 0q88.5 0 151 62.5T427 213q0 70-41 125.5T281 416q-14 2-14-11v-58q0-27-15-40q44-5 70.5-27t26.5-77q0-34-22-58q11-26-2-57q-18-5-58 22q-26-7-54-7t-53 7q-18-12-32.5-17.5T107 88h-6q-12 31-2 57q-22 24-22 58q0 55 27 77t70 27q-11 10-13 29q-42 18-62-18q-12-20-33-22q-2 0-4.5.5t-5 3.5t8.5 9q14 7 23 31q1 2 2 4.5t6.5 9.5t13 10.5T130 371t30-2v36q0 13-14 11q-64-22-105-77.5T0 213q0-88 62.5-150.5T213.5 0z" />
                            </svg>
                            <div className="text-center">
                                <h3 className="font-bold text-lg">GitHub</h3>
                                {/* <p className="text-sm text-muted-foreground">AppRouter/Layouts/APIRoutesの技術の経験。</p> */}
                            </div>
                        </div>
                    </div>
                </div>
            </section>

            <section id="contact" className="container mx-auto px-4 py-8 md:py-12 lg:py-12">
                <div className="max-w-[58rem] mx-auto text-center flex flex-col gap-4">
                    <h2 className="font-extrabold text-3xl md:text-6xl">Contact Me</h2>
                    <p className="text-muted-foreground sm:text-lg sm:leading-7">
                        お問い合わせは、下記のXアカウントにDMでご連絡ください。
                        <br />
                        ご連絡お待ちしております。
                    </p>
                    <Link href={siteConfig.links.x} className="underline underline-offset-4" target="_blank" rel="noreferrer">Xアカウントに問い合わせする</Link>
                </div>
            </section>
        </>
    )
}